...
(() => {
  ...
  window.PR_REVIEW_RETEST_PLAN = {
    ...
    commits,
    deployment: null,
    scope: 'Six critical/high defect regressions × six Chrome sizes. Original 30-scenario campaign remains unchanged; no campaign-wide completion claim.',
    ...
    scenarios: Array.from({length: 30}, (_, index) => ({
      id: 'KYCA-' + String(index + 1).padStart(3, '0'),
      sizes: sizes.map(size => ({size, status: 'PENDING', evidence: []})),
    })),
  };
  const summaries = {
    "KYCA-F001": "Prefer authorized signed document URLs with safe legacy fallback for readable Chrome evidence.",
    "KYCA-F002": "Block incomplete AIR verification in both UI and server, including required evidence, facts, consent and validation.",
    "KYCA-F003": "Trim compliance reasons and reject whitespace-only values in UI and API.",
    "KYCA-F004": "Make terminal approval records read-only while retaining authorized evidence access.",
    "KYCA-F005": "Keep the primary individual Review action visible in the sticky first queue column.",
    "KYCA-F006": "Keep the primary AIR View action visible in the sticky first queue column."
};
