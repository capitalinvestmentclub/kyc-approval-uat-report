...
test('prepared overlay preserves unresolved findings and exact pending Chrome matrix', () => {
 ...
 assert.equal(Object.keys(x.w.PR_REVIEW_UPDATES).length, 6);
...
function setup(query='', results){
 const dom=new JSDOM(html,{url:'https://capitalinvestmentclub.github.io/kyc-approval-uat-report/'+query,runScripts:'outside-only'});const w=dom.window,d=w.document;
...
test('publishes all approval scenarios, six-size cells and findings',()=>{const x=setup();assert.equal(x.all('.finding-row').length,7);assert.equal(x.all('#scenarios tr').length,30);assert.equal(x.all('.cells span').length,180);assert.equal(x.$('#metric-total').textContent,'7');assert.equal(x.$('#metric-urgent').textContent,'6');assert.match(x.d.body.textContent,/30\/30 scenarios closed/);assert.match(x.d.body.textContent,/Final UAT score: 3\/10/);x.dom.window.close();});
test('supports finding search and deep links',()=>{const x=setup();x.$('#search').value='whitespace';x.$('#search').dispatchEvent(new x.w.Event('input'));assert.equal(x.all('.finding-row').length,1);x.$('.finding-row').click();assert.ok(x.$('dialog').open);assert.match(x.$('#dialog-content').textContent,/moved from In Progress to Queried/i);x.dom.window.close();const y=setup('#KYCA-F002');assert.ok(y.$('dialog').open);assert.match(y.$('#dialog-title').textContent,/Incomplete AIR/);y.dom.window.close();});
